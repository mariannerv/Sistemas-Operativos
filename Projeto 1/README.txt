...
1) ./pgrepwc -t -c palavra ficheiro1.txt
2) ./pgrepwc -c -t -p 3 palavra ficheiro1.txt ficheiro2.txt ficheiro3.txt
3) ./pgrepwc -c -p 4 -e -t palavra ficheiro1.txt
...
Os ficheiros dados como argumentos foram colocados numa lista, sendo a divisão dessa lista efetuada através da formula listaProcesso=[round(len(ficheiros)/n*i):round(len(ficheiros)/n*(i+1))], realizando assim uma divisão justa por cada processo, certificando de que um ficheiro não é colocado numa lista mais do que uma vez, e certificando de que todos os ficheiros vão ficar numa lista
